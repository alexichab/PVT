...
void get_chunk(int a, int b, int commsize, int rank, int *lb, int *ub) {
  ...
  if (n % commsize) q++;
  int r = commsize * q - n;
  /*Вычисляем размер блока для процесса */
  int chunk = q;
  if (rank >= commsize - r) chunk = q - 1;
  *lb = a;        /* Определяем начальный элемент процесса */
  if (rank > 0) { /* Подсчитать сумму предыдущих блоков  */
    if (rank <= commsize - r) {
      *lb += q * rank;
    } else {
...
/* sgemv: вычисляем произведение матрицы на вектор c[m] = a[m][n] * b[n] */
...
int main(int argc, char **argv) {
  ...
  // Каждый процесс инициализирует свои массивы
  for (int i = 0; i < nrows; i++) {
    for (int j = 0; j < n; j++) {
      a[i * n + j] = lb + i + 1;
    }
